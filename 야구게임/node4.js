...
function getUserNum () {
  ...
  input.question(`${testCount}번째 시도 : `, function program (userValue) {
    ...
    // strike 및 ball 초기화
    let strike = 0; // strike : 숫자의 값과 위치가 일치한 경우
    let ball = 0;   // ball : 숫자의 값은 존재하지만, 위치가 틀렸을 경우
    testCount++     // 입력받은 횟수 업데이트 
    let userValueSplit = userValue.split("")  //  [1, 2. 3]

    // 랜덤숫자와 사용자 숫자의 비교 
    userValueSplit.forEach((num, index) => num === computer.charAt(index) // computer[i] >> 문자열 탐색 [i], undefined - 띄어쓰기:에러발생 >> 문자열탐색 charAt(i) >> 결과를 반환하지 않음 >> 에러발생 안됨
      ? strike++  // strike += 1
